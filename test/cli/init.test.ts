 ...
 * 
 * Tests for the spects init command that generates starter templates
 */
...
const spectsCmd = join(process.cwd(), 'bin/spects.js');
...
describe('FR-105: Project Initialization', () => {
  ...
  describe('FR-105-01: spects init creates design/ directory structure', () => {
    it('creates design/ directory', () => {
      execSync(`node ${spectsCmd} init`, { cwd: testDir });
  ...
  describe('FR-105-02: spects init generates spects.config.ts', () => {
    it('creates spects.config.ts file', () => {
      execSync(`node ${spectsCmd} init`, { cwd: testDir });
      
      expect(existsSync(join(testDir, 'spects.config.ts'))).toBe(true);
    });

    it('spects.config.ts imports from spects', () => {
      execSync(`node ${spectsCmd} init`, { cwd: testDir });
      
      const content = readFileSync(join(testDir, 'spects.config.ts'), 'utf-8');
      expect(content).toContain("import { defineConfig } from 'spects'");
    });

    it('spects.config.ts exports default config', () => {
      execSync(`node ${spectsCmd} init`, { cwd: testDir });
  ...
  describe('FR-105-03: spects init generates package.json', () => {
    ...
    it('package.json includes spects dependency', () => {
      execSync(`node ${spectsCmd} init`, { cwd: testDir });
      
      const packageJson = JSON.parse(readFileSync(join(testDir, 'package.json'), 'utf-8'));
      expect(packageJson.dependencies.spects).toBeDefined();
    });
  ...
  describe('FR-105-04: spects init generates tsconfig.json', () => {
  ...
  describe('FR-105-05: spects init generates basic model definitions', () => {
  ...
  describe('FR-105-06: spects init generates sample specification files', () => {
  ...
  describe('FR-105-07: Generated project passes spects lint', () => {
    it('spects lint succeeds on generated project', () => {
      execSync(`node ${spectsCmd} init`, { cwd: testDir });
      
      // Install dependencies (using local spects package)
      execSync(`npm install ${process.cwd()} --save`, { cwd: testDir });
      
      // Run spects lint
      const result = execSync(`node ${spectsCmd} lint`, { 
        cwd: testDir,
        encoding: 'utf-8',
      });
  ...
  describe('FR-105-09: spects init --force overwrites existing files', () => {
    it('overwrites spects.config.ts with --force', () => {
      // First init
      execSync(`node ${spectsCmd} init`, { cwd: testDir });
      
      // Modify config
      const configPath = join(testDir, 'spects.config.ts');
      writeFileSync(configPath, '// modified\n');
      
      // Second init with force
      execSync(`node ${spectsCmd} init --force`, { cwd: testDir });
  ...
  describe('FR-105-10: spects init skips package.json if exists (without --force)', () => {

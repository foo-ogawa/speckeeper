...
export function generateSpecDataFile(
  ...
  if (format === 'yaml') {
    const content = `# ${templateInfo.modelName} Specifications
#
# Generated by speckeeper scaffold — add your specs below.
model: ${templateInfo.fileName}
specs:
  # - id: ${templateInfo.defaultIdPrefix}-001
  #   name: "..."
  #   description: "..."
`;
    return {
...
export function generateAllModelFiles(
  ...
  for (const node of speckeeperNodes) {
    ...
    files.push(generateSpecDataFile(node, format));
  }

  files.push(generateDesignIndex(speckeeperNodes, format));

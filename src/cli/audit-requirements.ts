...
import { buildAuditRequirementsContext } from "../agents/context-builder.js";
import {
  runAgentTask,
  computeExitCode,
  formatResult,
...
export interface CommandAuditRequirementsOptions {
  config?: string;
  adapter?: string;
  model?: string;
  showPrompt?: boolean;
...
export async function commandAuditRequirements(
  opts: CommandAuditRequirementsOptions,
): Promise<void | string> {
  const context = await buildAuditRequirementsContext(opts.config);
